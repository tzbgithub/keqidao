    ...
    $.extend({
    	...
    	modal: {
            ...
            openOptions: function (options) {
                ...
                var _height = $.common.isEmpty(options.height) ? ($(window).height() - 50)+"px" : options.height;
                var _btn = $.common.isEmpty(options.height) ?  ['<em class="fa fa-check"></em> 确认', '<em class="fa fa-close"></em> 关闭']: options.btn;
        ...
        operate: {
			...
			logicRemoveAll: function() {
				...
				$.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function() {
					var url = $.table._option.logicRemoveUrl;
					var data = { "ids": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
			},
